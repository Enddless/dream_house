        ...
        <aside className={classNamesList} onMouseLeave={() => setUnhoverEffect(true)}>
            {showSuccess ? (
                <div className={css.notification}>
                    {count && count !== "" && <div className={css.circle}><p className={css.count}> {count}</p></div>}
                    <p className={css.atentionText}>Мы получили Ваше сообщение. Ответ направим на указанную почту</p>

                </div>
            ) : (
                <form onSubmit={submitForm}>
                    <fieldset>
                        <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Your mail" className={errorMail && errorMail !== "" ? `${css.errorInput}` : ""} />
                        {errorMail && errorMail !== "" && (
                    ...
                    <fieldset>
                        <input type="text" value={question} onChange={(e) => setQuestion(e.target.value)} placeholder="Your question" className={errorText && errorText !== "" ? `${css.errorInput}` : ""} />
                        {errorText && errorText !== "" && (
                    ...
                    <div className={css.buttonContainer}>
                        <button title="Заполните поля" className={css.discuss} disabled={!isValidForm}>
                            discuss a project
                        </button>
